...
class InventoryItemCreationListener {
	...
	private final @NonNull ObjectProvider<Inventory> inventory;
	...
	public void verify(InventoryItem item) {
		...
		var existing = inventory.getObject().findByProductIdentifier(item.getProductAssociation().getId());
	...
	private void assertNonUniqueItem(InventoryItem item) {
		...
		inventory.getObject().findByProductIdentifier(item.getProductAssociation().getId()) //
